...
class Jetpack_Video {
	...
	/*
	 * Bump Video > Published Projects stat when projects are published
	 */
	function new_project_stat_bump() {
		bump_stats_extras( 'videos', 'published-projects' );
	}
	...
	*/
	...
	function flush_rules_on_first_project() {
		$projects = get_transient( 'jetpack-video-count-cache' );
		...
		if ( false === $projects ) {
			...
			if ( ! empty( $projects ) ) {
				set_transient( 'jetpack-video-count-cache', $projects, HOUR_IN_SECONDS * 12 );
			}
		}
	}
 ...
 'read_post' => 'edit_posts',
	...
	function edit_admin_columns( $columns ) {
		// change 'Title' to 'Videos'
		$columns['title'] = __( 'Videos', 'jetpack' );
		if ( current_theme_supports( 'post-thumbnails' ) ) {
			// add featured image before 'Videos'
	...
	static function video_shortcode( $atts ) {
		...
		wp_enqueue_style( 'jetpack-video-style', plugins_url( 'css/video-shortcode.css', __FILE__ ), array(), '20140326' );
	...
	static function video_shortcode_html( $atts ) {
		...
		?>
			...
			while ( $query->have_posts() ) {
				...
				$post_id = get_the_ID();
				?>
				<div class="video-entry <?php echo esc_attr( self::get_project_class( $video_index_number, $atts['columns'] ) ); ?>">
	 ...
	 * @return string
	 */
	static function get_project_class( $video_index_number, $columns ) {
		...
		/**
		 * Filter the class applied to project div in the video
		 *
		 ...
		 *
		 */
		return apply_filters( 'video-project-post-class', implode( " ", $class ) , $video_index_number, $columns );
	}
	...
	/**
	 * Displays the author of the current video project.
	 *
	 * @return html
	 */
