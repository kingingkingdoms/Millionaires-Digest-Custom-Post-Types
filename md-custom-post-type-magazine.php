...
class Jetpack_Magazine {
	...
	function __construct() {
		...
		add_filter( 'rest_api_allowed_post_types',                                     array( $this, 'allow_magazine_rest_api_type' ) );
		$setting = Jetpack_Options::get_option_and_ensure_autoload( self::OPTION_NAME, '0' );
		// Bail early if Magazine option is not set and the theme doesn't declare support
		...
		add_image_size( 'jetpack-magazine-admin-thumb', 50, 50, true );
		add_action( 'admin_enqueue_scripts',                                           array( $this, 'enqueue_admin_styles'  ) );
		// register jetpack_magazine shortcode and magazine shortcode (legacy)
		add_shortcode( 'magazine',                                                    array( $this, 'magazine_shortcode' ) );
		add_shortcode( 'jetpack_magazine',                                            array( $this, 'magazine_shortcode' ) );
	 ...
	 * for the Magazine CPT setting.
	 *
	 * @return html
	 */
	function setting_html() {
		if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
			<p><?php printf( /* translators: %s is the name of a custom post type such as "jetpack-magazine" */ __( 'Your theme supports <strong>%s</strong>', 'jetpack' ), self::CUSTOM_POST_TYPE ); ?></p>
		<?php else : ?>
		...
		if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
			printf( '<p><label for="%1$s">%2$s</label></p>',
				...
				sprintf( __( 'Magazine pages display at most %1$s magazines', 'jetpack' ),
	...
	/*
	 * Bump Magazine > New Activation stat
	 */
	function new_activation_stat_bump() {
		bump_stats_extras( 'magazines', 'new-activation' );
	}
	/*
	 * Bump Magazine > Option On/Off stats to get total active
	 */
	function update_option_stat_bump( $old, $new ) {
		if ( empty( $old ) && ! empty( $new ) ) {
			bump_stats_extras( 'magazines', 'option-on' );
		}
		if ( ! empty( $old ) && empty( $new ) ) {
			bump_stats_extras( 'magazines', 'option-off' );
		}
	}
	/*
	 * Bump Magazine > Published Projects stat when projects are published
	 */
	function new_project_stat_bump() {
		bump_stats_extras( 'magazines', 'published-projects' );
	}
	/**
	...
	*/
	...
	function flush_rules_on_first_project() {
		$projects = get_transient( 'jetpack-magazine-count-cache' );
		if ( false === $projects ) {
			flush_rewrite_rules();
			$projects = (int) wp_count_posts( self::CUSTOM_POST_TYPE )->publish;
			if ( ! empty( $projects ) ) {
				set_transient( 'jetpack-magazine-count-cache', $projects, HOUR_IN_SECONDS * 12 );
			}
		}
	}
	/*
	...
	function register_post_types() {
		...
		register_post_type( self::CUSTOM_POST_TYPE, array(
			'description' => __( 'Magazine Items', 'jetpack' ),
			'labels' => array(
				'name'                  => esc_html__( 'Magazines',                   'jetpack' ),
				'singular_name'         => esc_html__( 'Magazine',                    'jetpack' ),
				'menu_name'             => esc_html__( 'Magazines',                  'jetpack' ),
				...
				'not_found'             => esc_html__( 'No Magazines found',          'jetpack' ),
				'not_found_in_trash'    => esc_html__( 'No Magazines found in Trash', 'jetpack' ),
				'filter_items_list'     => esc_html__( 'Filter magazines list',       'jetpack' ),
				'items_list_navigation' => esc_html__( 'Magazine list navigation',    'jetpack' ),
				'items_list'            => esc_html__( 'Magazines list',              'jetpack' ),
			),
			'supports' => array(
				'title',
				'editor',
 ...
 'read_post' => 'edit_posts',
	...
	/**
	 * Update messages for the Magazine admin.
	 */
	function updated_messages( $messages ) {
		global $post;
		$messages[self::CUSTOM_POST_TYPE] = array(
			0  => '', // Unused. Messages start at index 1.
			1  => sprintf( __( 'Magazine updated. <a href="%s">View item</a>', 'jetpack'), esc_url( get_permalink( $post->ID ) ) ),
			2  => esc_html__( 'Custom field updated.', 'jetpack' ),
			3  => esc_html__( 'Custom field deleted.', 'jetpack' ),
			4  => esc_html__( 'Magazine updated.', 'jetpack' ),
			/* translators: %s: date and time of the revision */
			5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Magazine restored to revision from %s', 'jetpack'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6  => sprintf( __( 'Magazine published. <a href="%s">View magazine</a>', 'jetpack' ), esc_url( get_permalink( $post->ID ) ) ),
			7  => esc_html__( 'Magazine saved.', 'jetpack' ),
			8  => sprintf( __( 'Magazine submitted. <a target="_blank" href="%s">Preview magazine</a>', 'jetpack'), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
			9  => sprintf( __( 'Magazine scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview magazine</a>', 'jetpack' ),
			// translators: Publish box date format, see http://php.net/date
			date_i18n( __( 'M j, Y @ G:i', 'jetpack' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post->ID ) ) ),
			10 => sprintf( __( 'Magazine item draft updated. <a target="_blank" href="%s">Preview magazine</a>', 'jetpack' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
		);
		return $messages;
	}
	/**
	...
	function edit_admin_columns( $columns ) {
		// change 'Title' to 'Magazines'
		$columns['title'] = __( 'Magazines', 'jetpack' );
		if ( current_theme_supports( 'post-thumbnails' ) ) {
			// add featured image before 'Magazines'
	...
	function image_column( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'thumbnail':
				echo get_the_post_thumbnail( $post_id, 'jetpack-magazine-admin-thumb' );
				break;
		}
	}
	/**
	...
	/**
	 * Adds magazine section to the Customizer.
	 */
	function customize_register( $wp_customize ) {
		...
		$wp_customize->add_section( 'jetpack_magazine', array(
			'title'                    => esc_html__( 'Magazine', 'jetpack' ),
			'theme_supports'           => self::CUSTOM_POST_TYPE,
			'priority'                 => 130,
		) );
		if ( isset( $options[0]['title'] ) && true === $options[0]['title'] ) {
			$wp_customize->add_setting( 'jetpack_magazine_title', array(
				'default'              => esc_html__( 'Magazines', 'jetpack' ),
				'type'                 => 'option',
				'sanitize_callback'    => 'sanitize_text_field',
				'sanitize_js_callback' => 'sanitize_text_field',
			) );
			$wp_customize->add_control( 'jetpack_magazine_title', array(
				'section'              => 'jetpack_magazine',
				'label'                => esc_html__( 'Magazine Archive Title', 'jetpack' ),
				'type'                 => 'text',
			) );
		}
		if ( isset( $options[0]['content'] ) && true === $options[0]['content'] ) {
			$wp_customize->add_setting( 'jetpack_magazine_content', array(
				'default'              => '',
				'type'                 => 'option',
				'sanitize_callback'    => 'wp_kses_post',
				'sanitize_js_callback' => 'wp_kses_post',
			) );
			$wp_customize->add_control( 'jetpack_magazine_content', array(
				'section'              => 'jetpack_magazine',
				'label'                => esc_html__( 'Magazine Archive Content', 'jetpack' ),
				'type'                 => 'textarea',
			) );
		}
		if ( isset( $options[0]['featured-image'] ) && true === $options[0]['featured-image'] ) {
			$wp_customize->add_setting( 'jetpack_magazine_featured_image', array(
				'default'              => '',
				'type'                 => 'option',
			...
			$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jetpack_magazine_featured_image', array(
				'section'              => 'jetpack_magazine',
				'label'                => esc_html__( 'Magazine Archive Featured Image', 'jetpack' ),
			) ) );
		}
	}
	/**
	 ...
	 * Add to REST API post type whitelist
	 */
	function allow_magazine_rest_api_type( $post_types ) {
		$post_types[] = self::CUSTOM_POST_TYPE;
		return $post_types;
	}
	/**
	 * Our [magazine] shortcode.
	 * Prints Magazine data styled to look good on *any* theme.
	 *
	 * @return magazine_shortcode_html
	 */
	static function magazine_shortcode( $atts ) {
		// Default attributes
		...
		if ( $atts['orderby'] ) {
			...
			$parsed = array();
			foreach ( explode( ',', $atts['orderby'] ) as $magazine_index_number => $orderby ) {
	...
	/**
	 * Query to retrieve entries from the Magazine post_type.
	 *
	 * @return object
	 */
	static function magazine_query( $atts ) {
	...
	/**
	 * The Magazine shortcode loop.
	 *
	 * @todo add theme color styles
	 * @return html
	 */
	static function magazine_shortcode_html( $atts ) {
		$query = self::magazine_query( $atts );
		$magazine_index_number = 0;
		ob_start();
		// If we have posts, create the html
		// with magazine markup
		if ( $query->have_posts() ) {
			// Render styles
			//self::themecolor_styles();
		?>
			<div class="jetpack-magazine-shortcode column-<?php echo esc_attr( $atts['columns'] ); ?>">
			<?php  // open .jetpack-magazine
			// Construct the loop...
			while ( $query->have_posts() ) {
				$query->the_post();
				$post_id = get_the_ID();
				?>
				<div class="magazine-entry <?php echo esc_attr( self::get_project_class( $magazine_index_number, $atts['columns'] ) ); ?>">
					<header class="magazine-entry-header">
					<?php
					// Featured image
					echo self::get_magazine_thumbnail_link( $post_id );
					?>

					<h2 class="magazine-entry-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_attr( the_title_attribute( ) ); ?>"><?php the_title(); ?></a></h2>

						<div class="magazine-entry-meta">
						<?php
				...
				if ( false !== $atts['display_content'] ) {
					...
					?>
						<div class="magazine-entry-content"><?php the_content(); ?></div>
					<?php
					} else {
					?>
						<div class="magazine-entry-content"><?php the_excerpt(); ?></div>
					<?php
					}
					remove_filter( 'wordads_inpost_disable', '__return_true', 20 );
				}
				?>
				</div><!-- close .magazine-entry -->
				<?php $magazine_index_number++;
			} // end of while loop
			wp_reset_postdata();
			?>
			</div><!-- close .jetpack-magazine -->
		<?php
		} else { ?>
			<p><em><?php _e( 'Your Magazine Archive currently has no entries. You can start creating them on your dashboard.', 'jetpack' ); ?></p></em>
		<?php
		}
		$html = ob_get_clean();
		// If there is a [magazine] within a [magazine], remove the shortcode
		if ( has_shortcode( $html, 'magazine' ) ){
			remove_shortcode( 'magazine' );
		}
	 ...
	 * @return string
	 */
	static function get_project_class( $magazine_index_number, $columns ) {
		$project_types = wp_get_object_terms( get_the_ID(), self::CUSTOM_TAXONOMY_TYPE, array( 'fields' => 'slugs' ) );
		$class = array();
		$class[] = 'magazine-entry-column-'.$columns;
		// add a type- class for each project type
		foreach ( $project_types as $project_type ) {
			$class[] = 'type-' . esc_html( $project_type );
		}
		if( $columns > 1) {
			if ( ( $magazine_index_number % 2 ) == 0 ) {
				$class[] = 'magazine-entry-mobile-first-item-row';
			} else {
				$class[] = 'magazine-entry-mobile-last-item-row';
			}
		}
		// add first and last classes to first and last items in a row
		if ( ( $magazine_index_number % $columns ) == 0 ) {
			$class[] = 'magazine-entry-first-item-row';
		} elseif ( ( $magazine_index_number % $columns ) == ( $columns - 1 ) ) {
			$class[] = 'magazine-entry-last-item-row';
		}
		/**
		 * Filter the class applied to project div in the magazine
		 *
		 ...
		 * @param int $magazine_index_number iterator count the number of columns up starting from 0.
		 * @param int $columns number of columns to display the content in.
		 *
		 */
		return apply_filters( 'magazine-project-post-class', implode( " ", $class ) , $magazine_index_number, $columns );
	}
	/**
	...
	/**
	 * Displays the author of the current magazine project.
	 *
	 * @return html
	 */
	 ...
	 * @return html
	 */
	static function get_magazine_thumbnail_link( $post_id ) {
		if ( has_post_thumbnail( $post_id ) ) {
			/**
			 * Change the Magazine thumbnail size.
			 *
			 * @module custom-content-types
			 *
			 * @since 3.4.0
			 ...
			 */
			return '<a class="magazine-featured-image" href="' . esc_url( get_permalink( $post_id ) ) . '">' . get_the_post_thumbnail( $post_id, apply_filters( 'jetpack_magazine_thumbnail_size', 'large' ) ) . '</a>';
		}
	}
}
add_action( 'init', array( 'Jetpack_Magazine', 'init' ) );
// Check on plugin activation if theme supports CPT
register_activation_hook( __FILE__,                         array( 'Jetpack_Magazine', 'activation_post_type_support' ) );
add_action( 'jetpack_activate_module_custom-content-types', array( 'Jetpack_Magazine', 'activation_post_type_support' ) );
