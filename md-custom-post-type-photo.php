...
class Jetpack_Photo {
	...
	/*
	 * Bump Photo > Published Projects stat when projects are published
	 */
	function new_project_stat_bump() {
		bump_stats_extras( 'photos', 'published-projects' );
	}
	...
	*/
	...
	function flush_rules_on_first_project() {
		$projects = get_transient( 'jetpack-photo-count-cache' );
		...
		if ( false === $projects ) {
			...
			if ( ! empty( $projects ) ) {
				set_transient( 'jetpack-photo-count-cache', $projects, HOUR_IN_SECONDS * 12 );
			}
		}
	}
 ...
 'read_post' => 'edit_posts',
	...
	function edit_admin_columns( $columns ) {
		// change 'Title' to 'Photos'
		$columns['title'] = __( 'Photos', 'jetpack' );
		if ( current_theme_supports( 'post-thumbnails' ) ) {
			// add featured image before 'Photos'
	...
	static function photo_shortcode( $atts ) {
		...
		wp_enqueue_style( 'jetpack-photo-style', plugins_url( 'css/photo-shortcode.css', __FILE__ ), array(), '20140326' );
	...
	static function photo_shortcode_html( $atts ) {
		...
		?>
			...
			while ( $query->have_posts() ) {
				...
				$post_id = get_the_ID();
				?>
				<div class="photo-entry <?php echo esc_attr( self::get_project_class( $photo_index_number, $atts['columns'] ) ); ?>">
	 ...
	 * @return string
	 */
	static function get_project_class( $photo_index_number, $columns ) {
		...
		/**
		 * Filter the class applied to project div in the photo
		 *
		 ...
		 *
		 */
		return apply_filters( 'photo-project-post-class', implode( " ", $class ) , $photo_index_number, $columns );
	}
	...
	/**
	 * Displays the author of the current photo project.
	 *
	 * @return html
	 */
