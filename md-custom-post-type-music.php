...
/**
 * Add custom post type support "Music" to Jetpack
 *
 * Class Jetpack_Music
 */
class Jetpack_Music {
	...
	const CUSTOM_POST_TYPE       = 'jetpack-music';
	...
	function __construct() {
		...
		add_action( sprintf( 'publish_%s', self::CUSTOM_POST_TYPE),                    array( $this, 'flush_rules_on_first_music' ) );
		...
		if ( defined( 'IS_WPCOM' ) && IS_WPCOM ) {
			...
			add_action( sprintf( 'publish_%s', self::CUSTOM_POST_TYPE),                array( $this, 'new_music_stat_bump' ) );
		}
	...
	/*
	 * Bump Music > Published Music stat when songs are published
	 */
	function new_music_stat_bump() {
		bump_stats_extras( 'music', 'published-music' );
	}
	/**
	...
	*/
	...
	/*
	 * Count published songs and flush permalinks when first songs is published
	 */
	function flush_rules_on_first_music() {
		$music = get_transient( 'jetpack-music-count-cache' );
		if ( false === $music ) {
			flush_rewrite_rules();
			$music = (int) wp_count_posts( self::CUSTOM_POST_TYPE )->publish;
			if ( ! empty( $music ) ) {
				set_transient( 'jetpack-music-count-cache', $music, HOUR_IN_SECONDS * 12 );
			}
		}
	}
	/*
 ...
 'read_post' => 'edit_posts',
	...
	static function music_shortcode( $atts ) {
		...
		wp_enqueue_style( 'jetpack-music-style', plugins_url( 'css/portfolio-shortcode.css', __FILE__ ), array(), '20140326' );
	...
	static function music_shortcode_html( $atts ) {
		...
		?>
			...
			while ( $query->have_posts() ) {
				...
				$post_id = get_the_ID();
				?>
				<div class="music-entry <?php echo esc_attr( self::get_music_class( $music_index_number, $atts['columns'] ) ); ?>">
					...
					<h2 class="music-entry-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_attr( the_title_attribute( ) ); ?>"><?php the_title(); ?></a></h2>
						...
						if ( false != $atts['display_types'] ) {
							echo self::get_music_type( $post_id );
						}
						if ( false != $atts['display_tags'] ) {
							echo self::get_music_tags( $post_id );
						}
						if ( false != $atts['display_author'] ) {
							echo self::get_music_author( $post_id );
						}
						?>
						</div>
	...
	/**
	 * Individual music class
	 *
	 * @return string
	 */
	static function get_music_class( $music_index_number, $columns ) {
		$music_types = wp_get_object_terms( get_the_ID(), self::CUSTOM_TAXONOMY_TYPE, array( 'fields' => 'slugs' ) );
		$class = array();
		$class[] = 'music-entry-column-'.$columns;
		// add a type- class for each music type
		foreach ( $music_types as $music_type ) {
			$class[] = 'type-' . esc_html( $music_type );
		}
		if( $columns > 1) {
		...
		/**
		 * Filter the class applied to song div in the song
		 *
		 ...
		 *
		 */
		return apply_filters( 'music-post-class', implode( " ", $class ) , $music_index_number, $columns );
	}
	/**
	 * Displays the song type that a song belongs to.
	 *
	 * @return html
	 */
	static function get_music_type( $post_id ) {
		$music_types = get_the_terms( $post_id, self::CUSTOM_TAXONOMY_TYPE );
		// If no types, return empty string
		if ( empty( $music_types ) || is_wp_error( $music_types ) ) {
			return;
		}
		$html = '<div class="music-types"><span>' . __( 'Types', 'jetpack' ) . ':</span>';
		$types = array();
		// Loop thorugh all the types
		foreach ( $music_types as $music_type ) {
			$music_type_link = get_term_link( $music_type, self::CUSTOM_TAXONOMY_TYPE );
			if ( is_wp_error( $music_type_link ) ) {
				return $music_type_link;
			}
			$types[] = '<a href="' . esc_url( $music_type_link ) . '" rel="tag">' . esc_html( $music_type->name ) . '</a>';
		}
		$html .= ' '.implode( ', ', $types );
		$html .= '</div>';
		return $html;
	...
	/**
	 * Displays the song tags that a song belongs to.
	 *
	 * @return html
	 */
	static function get_music_tags( $post_id ) {
		$music_tags = get_the_terms( $post_id, self::CUSTOM_TAXONOMY_TAG );
		// If no tags, return empty string
		if ( empty( $music_tags ) || is_wp_error( $music_tags ) ) {
			return false;
		}
		$html = '<div class="music-tags"><span>' . __( 'Tags', 'jetpack' ) . ':</span>';
		$tags = array();
		// Loop thorugh all the tags
		foreach ( $music_tags as $music_tag ) {
			$music_tag_link = get_term_link( $music_tag, self::CUSTOM_TAXONOMY_TYPE );
			if ( is_wp_error( $music_tag_link ) ) {
				return $music_tag_link;
			}
			$tags[] = '<a href="' . esc_url( $music_tag_link ) . '" rel="tag">' . esc_html( $music_tag->name ) . '</a>';
		}
		$html .= ' '. implode( ', ', $tags );
		$html .= '</div>';
		return $html;
	...
	/**
	 * Displays the author of the current song.
	 *
	 * @return html
	 */
	static function get_music_author() {
		$html = '<div class="music-author">';
